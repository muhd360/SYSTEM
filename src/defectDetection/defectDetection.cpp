...
int main(int argc, char* argv[])
{
	...
	Mat image = imread(inputImageName, cv::IMREAD_GRAYSCALE);
...
void processImage(Mat& image, double g_thresh, int mode)
{
	...
	case 2:
		...
		break;
	case 3:
		adaptiveThreshold(image, image, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C, cv::THRESH_BINARY_INV, 5, 5);
		info = "Use adaptive thresholding";
		break;
	case 0:
		equalizeHist(image, image);
		GaussianBlur(image, image, Size(5, 5), 3);
		threshold(image, image, g_thresh, 255, cv::THRESH_BINARY_INV);
...
vector<vector<Point> > getContours(Mat image)
{
	vector<vector<Point> > contours;
 	findContours(image, contours, cv::RETR_LIST, cv::CHAIN_APPROX_NONE); //CV_RETR_EXTERNAL
	return contours;
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	*/
	...
	Mat result;
	cvtColor(image, result, cv::COLOR_GRAY2BGR);
	imwrite(string("..\\data\\result\\") + string(ss.str()) + string(".png"), result);
	cout<<defect.getDiameter() << " -> " << refined_diameter << endl;
	namedWindow("result", 1);;
	circle(result, refined_center, cvRound(refined_radius), cv::Scalar(255, 0, 0));
...
void refineDefectSize(Mat image, Defect& defect, int y0, int x0){
	...
	namedWindow("result", 1);
	circle(result, Point2f(image.cols/2.0, image.rows/2.0), refined_radius, cv::Scalar(255, 0, 0));
	imshow("result", result);
	waitKey();
#endif
...
void displayResult(Mat image, vector<vector<Point>> contours, String outputImageName, double field_width, double defect_size)
{
	...
	for (int i = 0; i < int(contours.size()); i++) {
		...
		circle(result, center, 0.5 * diameter, cv::Scalar(255, 0, 0));
	}
	...
	for (int i = 0; i < 5; i++)
	{
		...
		if (showTextRight) {
			putText(result, string(ss1.str()) + ": ", Point(center.x + diameter/2 + 5, center.y + offset_vert * diameter/2 + showTextBottom * 25), cv::FONT_HERSHEY_SIMPLEX, 0.01, cv::Scalar(0, 0, 255));
			putText(result, string(ss.str()) + "mm", Point(center.x + diameter/2 + 45, center.y + offset_vert * diameter/2 + showTextBottom * 25), cv::FONT_HERSHEY_SIMPLEX, 0.01, cv::Scalar(255, 0, 0));
		}
		else {
			putText(result, string(ss1.str()) + ": ", Point(center.x - diameter/2 - 190, center.y + offset_vert * diameter/2 + showTextBottom * 25), cv::FONT_HERSHEY_SIMPLEX, 0.01, cv::Scalar(0, 0, 255));
			putText(result, string(ss.str()) + "mm", Point(center.x - diameter/2 - 150, center.y + offset_vert * diameter/2 + showTextBottom * 25), cv::FONT_HERSHEY_SIMPLEX, 0.01, cv::Scalar(255, 0, 0));
		}
